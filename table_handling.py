def get_ECG_EOG_chnames_by_SubjID(Subj_ID):
    ...
    if ECG_name == 0.0:
        print("For" + Subj_ID +
              " ECG channel name equals 0.0. probably \
              it was defined with a formula in excel table")
    if EOG_names == 0.0:
        print("For" + Subj_ID +
              " EOG channel name equals 0.0. probably it \
               was defined with a formula in excel table")
    ...
    # if EOG_names == "EOG061,EOG062" or \
       # EOG_names == "EMG065,EMG066,EMG067,EMG068":
    return ECG_name, EOG_names
    # else:
        # print("EOG ch_name " + EOG_names +
        #       " is bad (probably, there's space after comma)")
        # return
...
def get_eo_ec_by_name(Subj_ID, cond):
    ...
    while CondStart:
        ...
        CondEnd = sheet.cell_value(iSID, iCondStart + 1)
        if CondStart:
          CondStart = float(CondStart)
          CondEnd = float(CondEnd)
